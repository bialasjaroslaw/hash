...
class md5_hasher {
 public:
  ...
  void finalize() {
    ...
    update(md5_details::PADDING, padding_length);
  ...
  constexpr void fun(FN f, uint32_t& w, uint32_t x, uint32_t y, uint32_t z,
                     uint32_t in, uint32_t s) {
    w += f(x, y, z) + in;
    w = rotate_left(w, s) + x;
  }

  void step(uint32_t* buffer, const uint32_t* input) {
    using namespace md5_details;
    uint32_t a = buffer[0];
    uint32_t b = buffer[1];
    uint32_t c = buffer[2];
    uint32_t d = buffer[3];
 ...
 private:
  uint64_t _size = 0;
  uint32_t _buffer[4] = {md5_details::A, md5_details::B, md5_details::C,
                         md5_details::D};
  uint8_t _input[64] = {};
  uint8_t _digest[16] = {};
};
