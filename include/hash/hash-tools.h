...
inline std::vector<uint8_t> from_string(const std::string& str) {
  ...
  auto is_hex = [&](char maybe_hex) {
    return in_range(maybe_hex, '0', '9') || in_range(maybe_hex, 'a', 'f') ||
           in_range(maybe_hex, 'A', 'F');
  };
  if (!std::all_of(str.cbegin(), str.cend(), is_hex)) return {2};
  std::vector<uint8_t> hash;
  hash.reserve(str.size() / 2);
  auto hex_to_value = [&](char val) {
    return static_cast<uint8_t>(in_range(val, '0', '9')   ? val - '0'
                                : in_range(val, 'a', 'f') ? val - 'a' + 10
                                                          : val - 'A' + 10);
  };
  uint64_t idx = 0;
  uint8_t val = 0;
